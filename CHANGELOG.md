...
- **Plugin settings UI cleaned up.** Dispatcharr renders info-field descriptions as plain dimmed text that collapses newlines, so the boxes were rewritten as short flowing prose (Quick Start / Won't start? / Hydration) matching the official plugin convention — no ASCII banners.
- **Dependency-install failures now fail loudly.** Missing `uvicorn`/`fastapi`/`jinja2` and a failed auto-install make Enable/Status **raise** (a red notification — Dispatcharr only reddens on a raised exception) with the exact, copy-pasteable `docker exec <container> /dispatcharrpy/bin/python -m pip install …` command (container name filled in) and the real pip error, instead of silently spawning a child that crashes on `import uvicorn`. Documented in the README and the plugin's "Won't start?" box.

### Verified
- **Token-based auth (`enable_auth`)** exercised end-to-end on dev: no key / wrong key / wrong scheme → 401 (with `WWW-Authenticate: ApiKey`), valid `X-API-Key` and `Authorization: ApiKey <key>` → 200, public `/healthz` stays open. Matches Dispatcharr's own `ApiKeyAuthentication` semantics (`User.api_key`, `is_active`).
